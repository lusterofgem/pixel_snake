...
class PlayingPauseAnimation extends BaseAnimation {
  ...
  GameState? targetGameState = GameState.pause;

  // The start center position of the full screen animation
  Offset startCenter = Offset(6, 5);
  ...
  Size startSize = Size(10, 7);
  // The end size of the to full screen animation
  Size endSize = Size(80, 80);

  // The start color of the animation
  Color startColor = Color(0xFFD0E159);
  ...
  void renderOnCanvas(Canvas canvas, Size screenSize) {
    ...
    // Warning when the frame index is invalid but this function is called
      ...
      print("Warning: PlayingPauseAnimation::renderOnCanvas(Canvas, Size) called, but the frameIndex: ${frameIndex} is invalid.");
    }
  }
  ...
  double _toAbsoluteWidth(double relativeWidth) {
    ...
    if(screenSize == null) {
      print("Error: screenSize need to be set before PlayingPauseAnimation::_toAbsoluteWidth(double relativeWidth) being invoked.");
      return 0;
    }
  ...
  double _toAbsoluteHeight(double relativeHeight) {
    final screenSize = this.screenSize;
    if(screenSize == null) {
      print("Error: screenSize need to be set before PlayingPauseAnimation::_toAbsoluteHeight(double relativeHeight) being invoked.");
      return 0;
    }
